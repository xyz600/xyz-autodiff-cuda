...
// Overload for dividing a variable by a scalar constant
...
struct DivConstantLogic {
    ...
    T constant_c;  // The constant to divide by
    
    __host__ __device__ explicit DivConstantLogic(T c) : constant_c(c) {}
...
// Factory function for dividing by a constant
template <typename Input>
requires UnaryLogicParameterConcept<Input>
__device__ auto div_constant(Input& input, typename Input::value_type constant) {
    using LogicType = DivConstantLogic<Input>;
