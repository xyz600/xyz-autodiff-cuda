...
// Element-wise add with constant array: output[i] = input[i] + constant_array[i]
template <std::size_t Dim, typename ConstantArray>
requires ArrayLikeConcept<ConstantArray>
struct ConstArrayAddLogic {
    static constexpr std::size_t outputDim = Dim;
    
    const ConstantArray& constant_array;
    
    __host__ __device__ explicit ConstArrayAddLogic(const ConstantArray& arr) : constant_array(arr) {}
...
// Factory function for element-wise add with constant array
template <DifferentiableVariableConcept Input, typename ConstantArray>
requires ArrayLikeConcept<ConstantArray>
__host__ __device__ auto const_add(Input& input, const ConstantArray& constant_array) {
    constexpr std::size_t Dim = Input::size;
    ConstArrayAddLogic<Dim, ConstantArray> logic(constant_array);
    
    return UnaryOperation<Dim, ConstArrayAddLogic<Dim, ConstantArray>, Input>(logic, input);
}
